...
describe("Autenticación", () => {
    ...
    test("login con credenciales incorrectas → 401", async () => {
        const res = await request(app).post("/auth/login").send({ username: "admin", password: "malo" });
        assert.equal(res.status, 401);
    });
    ...
    test("/auth/me con token devuelve el usuario", async () => {
    ...
    test("ruta protegida con token válido → 200", async () => {
        const res = await request(app).get("/items").set(auth(adminToken));
        assert.equal(res.status, 200);
...
describe("Permisos por rol", () => {
    ...
    test("admin crea un operador", async () => {
        const res = await request(app).post("/usuarios").set(auth(adminToken))
            .send({ username: "operador_test", password: "oper123", nombre: "Op Test", rol: "OPERADOR" });
        assert.equal(res.status, 201);
        operadorId = res.body.id;

        const login = await request(app).post("/auth/login").send({ username: "operador_test", password: "oper123" });
        operadorToken = login.body.token;
        assert.ok(operadorToken);
    });

    test("operador puede crear (POST) un bombero", async () => {
        const res = await request(app).post("/bomberos").set(auth(operadorToken)).send({ nombre: "Bombero Op" });
        assert.equal(res.status, 201);
    });

    test("operador NO puede eliminar (DELETE) → 403", async () => {
        const res = await request(app).delete("/bomberos/999999").set(auth(operadorToken));
        assert.equal(res.status, 403);
    });

    test("operador NO puede gestionar usuarios → 403", async () => {
        const res = await request(app).get("/usuarios").set(auth(operadorToken));
        assert.equal(res.status, 403);
    });

    test("admin puede eliminar el operador", async () => {
        const res = await request(app).delete(`/usuarios/${operadorId}`).set(auth(adminToken));
        assert.equal(res.status, 200);
    });
});
...
describe("CRUD y validaciones de ítems", () => {
    test("crear ítem válido → 201", async () => {
        const res = await request(app).post("/items").set(auth(adminToken))
    ...
    test("filtro de estado inválido en búsqueda → 400", async () => {
        const res = await request(app).get("/items?estado=NOEXISTE").set(auth(adminToken));
        assert.equal(res.status, 400);
    });
});
...
describe("Unicidad de nombres", () => {
    test("bombero con nombre duplicado → 409", async () => {
        await request(app).post("/bomberos").set(auth(adminToken)).send({ nombre: "Nombre Unico" });
        const res = await request(app).post("/bomberos").set(auth(adminToken)).send({ nombre: "Nombre Unico" });
        assert.equal(res.status, 409);
    });

    test("ubicación duplicada → 409 y genera codigo_qr UBIC-", async () => {
        const r1 = await request(app).post("/ubicaciones").set(auth(adminToken)).send({ nombre: "Bodega Test", tipo: "BODEGA" });
        assert.equal(r1.status, 201);
        const ficha = await request(app).get(`/ubicaciones/${r1.body.id}`).set(auth(adminToken));
        assert.match(ficha.body.codigo_qr, /^UBIC-\d{4}$/);

        const r2 = await request(app).post("/ubicaciones").set(auth(adminToken)).send({ nombre: "Bodega Test", tipo: "BODEGA" });
...
describe("Integridad referencial (TRAUMA + usos)", () => {
    test("eliminar ítem TRAUMA con usos registrados → 200 (no falla por FK)", async () => {
        const crea = await request(app).post("/items").set(auth(adminToken))
            .send({ codigo: "TRA-TEST", categoria: "TRAUMA", descripcion: "Material trauma" });
        const itemId = crea.body.id;

        const uso = await request(app).post(`/trauma/${itemId}/usos`).set(auth(adminToken))
            .send({ fecha: "2026-06-01", cantidad: 2, motivo: "test" });
        assert.equal(uso.status, 201);

        const del = await request(app).delete(`/items/${itemId}`).set(auth(adminToken));
...
describe("Protección del último administrador", () => {
    test("no se puede eliminar el único admin → 400", async () => {
        const usuarios = await request(app).get("/usuarios").set(auth(adminToken));
        const admin = usuarios.body.find(u => u.username === "admin");
        const res = await request(app).delete(`/usuarios/${admin.id}`).set(auth(adminToken));
        assert.equal(res.status, 400);
    });
});
